...
ALL_CLASSES = STATICS_CLASSES + DYNAMICS_CLASSES
...
def test_runner_and_evaluator():
    ...
    result = runner.run([statics_suite, dynamics_suite])
...
def test_random_baseline_scores_less_than_oracle():
    ...
    oracle_result = Runner(PhysicsOraclePredictor()).run([statics_suite, dynamics_suite])
    random_result = Runner(RandomPredictor(seed=7)).run([statics_suite, dynamics_suite])
